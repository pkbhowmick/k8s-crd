...
type KubeApiLister interface {
	...
	List(selector labels.Selector) (ret []*v1.KubeApi, err error)
...
// List lists all KubeApis in the indexer.
func (s *kubeApiLister) List(selector labels.Selector) (ret []*v1.KubeApi, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.KubeApi))
	})
	return ret, err
}
...
type KubeApiNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.KubeApi, error)
...
// List lists all KubeApis in the indexer for a given namespace.
func (s kubeApiNamespaceLister) List(selector labels.Selector) (ret []*v1.KubeApi, err error) {
...
// Get retrieves the KubeApi from the indexer for a given namespace and name.
func (s kubeApiNamespaceLister) Get(name string) (*v1.KubeApi, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("kubeapi"), name)
	}
	return obj.(*v1.KubeApi), nil
}
